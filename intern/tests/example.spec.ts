...
test("login", async ({ page }) => {
  ...
  await page.fill('input[name="email"]', "sarahmontegro@gmail.com");
  await page.fill('input[name="password"]', "radek480");
  ...
  await page.waitForFunction(
    () => {
      const chart = document.querySelector(
        '[data-testid="zoom-poll-chart"] canvas'
      );
      return chart !== null;
    },
    null,
  ...
  const isChartVisible = await page.isVisible(
    '[data-testid="zoom-poll-chart" canvas]'
  );
