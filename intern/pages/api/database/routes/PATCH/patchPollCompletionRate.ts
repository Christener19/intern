...
async function handlePatchRequest(req: NextApiRequest, res: NextApiResponse) {
  try {
    ...
    const pollCompletionRate = await engagementController.patchPollCompletion(
      zoomId,
      data.week_number,
      data.poll_completion_rate,
      testQuery
    );

    // If the registration fails, return a 404 response
    if (!pollCompletionRate) {
      return res
        .status(404)
    ...
    res.status(200).json({ status: "success", data: pollCompletionRate });
  } catch (error) {
    // Handle errors and return a 500 response
    console.error("Error in patchPollCompletionRate:", error);
