...
async function handlePatchRequest(req: NextApiRequest, res: NextApiResponse) {
  try {
    ...
    const screenShareSwitchRate = await engagementController.patchScreenShareSwitchFreq(
      zoomId,
      data.week_number,
      data.screen_share_switch_freq,
      testQuery
    );

    // If the registration fails, return a 404 response
    if (!screenShareSwitchRate) {
      return res
        .status(404)
    ...
    res.status(200).json({ status: "success", data: screenShareSwitchRate });
  } catch (error) {
    // Handle errors and return a 500 response
    console.error("Error in patchScreenShareSwitchFreq:", error);
